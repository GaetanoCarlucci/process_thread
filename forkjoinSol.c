...
int main( ){ 
  ...
  wait(&status);                     
  // leggere solo il byte piu' significativo della variabile status 
  x = status;                        
  // suggerimento: eseguire uno shift di status a sinistra di 8 posizioni
  x = status >> 8;
  z = calcoli_finali_padre(x, y);           // chiamare la funzione che esegue il calcolo finale e lo scrive in z
